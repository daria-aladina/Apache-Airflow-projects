...
<p dir="auto">Задача: Создать DAG в Airflow с ETL-пайплайном, который будет считаться каждый день за вчера. Требования к пайплайну:</p>
  <ol>
    <li>1. Необходимо параллельно обрабатывать две таблицы. В feed_actions для каждого юзера считать: число просмотров и лайков контента. В message_actions для каждого юзера считать: количество отправленных сообщений, количество                   отправленных   сообщений, скольким людям он пишет, сколько людей пишет ему. Каждую выгрузку необходимо делать отдельным таском.</li>
    <li>2. Объединить две полученные таблицы в одну.</li>
    <li>3. Для получившейся общей таблицы необходимо посчитать все эти метрики в разрезе по полу, возрасту и ос. Рассчёт метрик по каждому срезу должен быть в отдельном таске.</li>
    <li>4. Финальные данные со всеми метриками записывать в отдельную таблицу в ClickHouse.</li>
    <li>5. Каждый день таблица должна наполняться новыми данными</li>
  </ol>
